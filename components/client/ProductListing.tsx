...
export default function ProductListing({
  ...
  const [categoryId, setCategoryId] = useState<string | "all">(initialCategoryId as any);
  ...
  const filtered = useMemo(() => {
    ...
    return (initialProducts || [])
      .filter((p) => {
        ...
        const matchesText = !q || p.name?.toLowerCase().includes(q) || p.brand?.name?.toLowerCase().includes(q);
        // category (support either flat categoryId or nested category.id)
        const matchesCat =
          categoryId === "all" || p.categoryId === categoryId || p.category?.id === categoryId;
        // price
        const price = p.variants?.[0]?.price as number | undefined;
        const matchesPrice = (min === undefined || (price ?? Infinity) >= min) && (max === undefined || (price ?? -Infinity) <= max);
        // stock
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-3 md:items-end">
        <div className="flex-1">
          ...
          <Input value={query} onChange={(e) => { setQuery(e.target.value); resetPagination(); }} placeholder="Search products" />
        </div>
        <div className="w-48">
          <label className="text-xs text-gray-600">Category</label>
          <Select value={categoryId} onValueChange={(v) => { setCategoryId(v as any); resetPagination(); }}>
            <SelectTrigger>
        ...
        <div className="w-40">
          ...
          <Select value={availability} onValueChange={(v) => { setAvailability(v as any); resetPagination(); }}>
            <SelectTrigger>
        ...
        <div className="w-28">
          ...
          <Input inputMode="decimal" value={minPrice} onChange={(e) => { setMinPrice(e.target.value); resetPagination(); }} placeholder="0" />
        </div>
        <div className="w-28">
          <label className="text-xs text-gray-600">Max price</label>
          <Input inputMode="decimal" value={maxPrice} onChange={(e) => { setMaxPrice(e.target.value); resetPagination(); }} placeholder="999" />
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {pageItems.map((p) => {
            const cover =
              ...
              p.images?.[0]?.imageUrl ||
              "/next.svg";
            const isRemote = typeof cover === "string" && /^https?:\/\//i.test(cover);
            ...
            return (
              <Card key={p.id} className="overflow-hidden">
                <div className="relative h-36 w-full bg-gray-50">
                  <Image src={cover} alt={p.name} fill className="object-contain p-3" unoptimized={isRemote} />
                </div>
                <CardHeader>
                ...
                <CardContent>
                  ...
                  {price != null && (
                    <p className="mt-2 text-gray-900 font-medium">${price.toFixed(2)}</p>
                  )}
                </CardContent>
                <CardFooter className="flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={!!addingIds[p.id] || (Number(p.variants?.[0]?.stockQuantity ?? 0) <= 0)}
                  >
